...
function kirim() {
  ...
  if (uang === null || uang === "") {
    alert("Uang tidak mencukupi, mohon maaf pembayaran dibatalkan.");
    return;
  }
  uang = parseInt(uang);
  
  if (isNaN(uang)) {
    alert("Inputan uang tidak valid.");
    return;
  }
  ...
  if (kembalian >= 0) {
    ...
    alert("Uang yang diberikan tidak mencukupi nominal pembelanjaan.");
  }
}